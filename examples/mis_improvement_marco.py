...
class MISImprovementProblem(ImprovementProblem):
    ...
    def _obj_function(self, state: State) -> torch.Tensor:
        ...
        obj_value = state.solutions.sum(2).float()
...
memory_type = 'none'  # 'marco' or 'last_action' or 'none'
mis_memory = select_memory(memory_type=memory_type,
                           mem_aggr='linear',
...
mis_model = EdgeInGTModel(node_in_dim=node_in_dim, node_out_dim=1, edge_in_dim=1, decoder='linear', aux_node=False,
...
mis_trainer.inference(problem_size=problem_size, batch_size=batch_size, pomo_size=pomo_size, deterministic=True, seed=42, verbose=True)
mis_trainer.train(epochs=10, episodes=10, problem_size=problem_size, batch_size=batch_size, pomo_size=pomo_size, eval_problem_size=problem_size,
